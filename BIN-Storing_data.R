...
#increments only 1:  c(autoincrement(philDB$books), autoincrement(philDB$books)
#chose to use seq() because it would be useful when adding many new entries at once
y <- data.frame(id = seq(autoincrement(philDB$books), autoincrement(philDB$books)+1),
...
philDB$person <- rbind(philDB$person, x)
philDB$books <- rbind(philDB$books,y)

z <- data.frame(id = seq(autoincrement(philDB$works), autoincrement(philDB$works)+1),
                personID = rep(which(philDB$person$name == "Immanuel Kant"), 2),
                bookID = c(which(philDB$books$title == "Critique of Pure Reason"),
                           which(philDB$books$title == "Critique of Judgement"))
)

philDB$works <- rbind(philDB$works, z)
